...
async def sync_data(storage, table):
    try:
        api_root = (
        ...
        for item in match_data:
            ...
            if not storage.check_match(item["matchId"]):
                storage.insert_match(match, table=table)
...
async def main():
    table = input("Enter the table name: ") or "matches"
    db_params = {
    ...
    while True:
        try:
            ...
            if current_minute % 10 == 0:
                print("Syncing data...")
                matches = await sync_data(storage, table)
                print(matches)
